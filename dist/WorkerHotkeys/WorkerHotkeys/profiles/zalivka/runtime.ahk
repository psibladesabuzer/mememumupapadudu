...
; --- functions ---
HK_1() {
    LogWrite("HOTKEY ^Numpad1 Пароль #1 для шеллов")
    Send("{Tab}")
    Sleep(100)
...
HK_2() {
    LogWrite("HOTKEY ^Numpad2 Пароль #2 для шеллов")
    Send("{Tab}")
    Sleep(100)
    Send("=ghjcnbnenrf=")
    Sleep(100)
...
HK_3() {
    LogWrite("HOTKEY ^Numpad3 Пароль #3 для шеллов")
    Send("{Tab}")
    Sleep(100)
...
HK_5() {
    LogWrite("HOTKEY ^e Ввод стандартного логина (admin) и пароля (ДЛЯ ВХОДА В АДМИНКУ)")
    Send("wpadmin")
    Sleep(100)
    Send("{Tab}")
    Sleep(100)
...
HK_6() {
    LogWrite("HOTKEY ^!#p Скриншот")
    MsgBox("")
}

HK_7() {
    LogWrite("HOTKEY ^#!1 Заливка")
    MsgBox("")
}

HK_8() {
    LogWrite("HOTKEY ^!#2 Sitemap")
    MsgBox("")
}

HK_9() {
    LogWrite("HOTKEY ^!#3 Hide")
    MsgBox("")
}

HK_10() {
    LogWrite("HOTKEY !#^4 Rollback/Force-Delete")
    MsgBox("")
}

; --- registration ---
Hotkey("$*^Numpad1", RunHotkeySafely.Bind(HK_1), "On")
Hotkey("$*^Numpad2", RunHotkeySafely.Bind(HK_2), "On")
Hotkey("$*^Numpad3", RunHotkeySafely.Bind(HK_3), "On")
Hotkey("$*^q", RunHotkeySafely.Bind(HK_4), "On")
Hotkey("$*^e", RunHotkeySafely.Bind(HK_5), "On")
Hotkey("$*^!#p", RunHotkeySafely.Bind(HK_6), "On")
Hotkey("$*^#!1", RunHotkeySafely.Bind(HK_7), "On")
Hotkey("$*^!#2", RunHotkeySafely.Bind(HK_8), "On")
Hotkey("$*^!#3", RunHotkeySafely.Bind(HK_9), "On")
Hotkey("$*!#^4", RunHotkeySafely.Bind(HK_10), "On")
RegisterGeneratedHotkeys()
